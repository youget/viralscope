...
const DEFAULT_MSG = { role: 'assistant', content: "Yo, sup! Pick a mode and ask me anything — I'm only slightly unhinged." }
...
const ALL_IMG_MODELS = [
  ...
  { id: 'p-image',    label: 'Pruna p-image',     type: 'byop' },
  { id: 'klein',      label: 'FLUX.2 Klein 4B',   type: 'byop' },
  { id: 'gptimage',   label: 'GPT Image 1 Mini',  type: 'byop' },
  { id: 'qwen-image', label: 'Qwen Image Plus',   type: 'byop' },
  { id: 'wan-image',  label: 'Wan 2.7 Image',     type: 'byop' },
  { id: 'kontext',    label: 'FLUX.1 Kontext',    type: 'byop' },
]
...
const ERR = {
  quota_exceeded: { emoji: '😭', title: "Pollen depleted", desc: "Your API key has run out of pollen. Top up at enter.pollinations.ai" },
  invalid_key:    { emoji: '🫠', title: "That key ain't it chief", desc: "Double check your API key and try again." },
  user_key_required: { emoji: '🎟️', title: "Key required", desc: "This feature needs your own API key." },
...
export default function AIPage() {
  ...
  function handleKeyClear() { clearUserKeyStorage(); setUserKey('') }
  ...
  function handleFileSelect(e) {
    ...
    if (!f.type.startsWith('image/')) { toast('Select an image file'); return }
    setChatImage(f); const r = new FileReader(); r.onload = (ev) => setChatImagePreview(ev.target.result); r.readAsDataURL(f)
  }
  ...
  async function doChat(text) {
    ...
    try {
      ...
      setChatMessages([...newDisplay, { role: 'assistant', content: data.result || 'Aduh, gw ga punya jawaban nih.' }])
    } catch {
      setChatMessages([...newDisplay, { role: 'assistant', content: 'Ada yang error. Coba lagi ya?' }])
    }
  ...
  async function doGenerate(overrideSeed) {
    ...
    const isFreeModel = currentImgModel.type === 'free'
    ...
    try {
      const fetchOpts = isFreeModel ? {} : { headers: { 'Authorization': `Bearer ${getUserKey()}` } }
      const res = await fetch(imageUrl, fetchOpts)

      if (!res.ok) {
        if (res.status === 429) { setImgError('Rate limit hit. Wait a moment and try again.'); setImgLoading(false); return }
  ...
  async function handleVoiceGenerate() {
    ...
    const k = getUserKey(); if (!k) { requireKey('voice', k2 => doVoice(k2)); return }
    doVoice(k)
  }
  ...
  async function handleVideoGenerate() {
    ...
    const k = getUserKey(); if (!k) { requireKey('video', k2 => doVideo(k2)); return }
    doVideo(k)
  }
  ...
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto">
      ...
      <p className="text-xs vs-text-sub text-center mb-5">create unhinged stuff with artificial brainpower</p>
      ...
      {tab === 'chat' && (
        <div className="flex flex-col" style={{ height: 'calc(100vh - 300px)' }}>
          ...
          <button onClick={handleClearChat} className="text-[10px] vs-text-sub hover:underline mb-2 self-end">Hapus riwayat</button>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-3 mb-4 pr-1">
            {chatMessages.map((msg, i) => (
              <div key={i} className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'self-end text-white rounded-br-sm' : 'self-start vs-card border vs-border vs-text rounded-bl-sm'}`}
                ...
                {typeof msg.content === 'string' ? msg.content.split('\n').map((l, j) => <span key={j}>{l}<br /></span>) : String(msg.content)}
              </div>
            ))}
            {chatLoading && (
          ...
          {chatImagePreview && (
            <div className="flex items-center gap-2 mb-2 px-2 py-1.5 vs-card border vs-border rounded-lg">
              <img src={chatImagePreview} alt="" className="w-10 h-10 rounded object-cover" />
              <span className="text-[10px] vs-text-sub flex-1">Gambar dilampirkan</span>
          ...
          <form onSubmit={handleChat} className="flex gap-2">
            {currentChatModel.vision && (
              <>
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()}
                  className="w-11 h-11 rounded-xl vs-card border vs-border flex items-center justify-center flex-shrink-0 vs-hover"
                  style={{ color: chatImage ? 'var(--vs-accent)' : 'var(--vs-text-sub)' }}>
                  <Paperclip size={16} />
                </button>
              </>
            )}
            ...
            <button type="submit" disabled={chatLoading} className="vs-btn w-11 h-11 rounded-xl flex-shrink-0">
              <Send size={16} />
            </button>
          </form>
          <p className="text-[9px] vs-text-sub mt-1.5 text-center">
            Tekan Enter untuk kirim{currentChatModel.vision ? ' • 📎 Lampirkan gambar di Pro mode' : ''}
          </p>
        </div>
      )}
      ...
      {tab === 'image' && (
        <div>
          <div className="mb-4">
            ...
            <p className="text-[10px] vs-text-sub mt-1.5">✨ Free • 🔑 Requires API key</p>
          </div>
      ...
      {tab === 'voice' && (
        <div>
          <div className="flex gap-2 mb-5">
            {[['tts', 'TTS', Mic], ['music', 'Music', Music]].map(([mode, label, Icon]) => (
              <button key={mode} onClick={() => setVoiceMode(mode)} className="flex-1 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5"
          ...
          <div className="mb-4">
            <p className="text-xs font-semibold vs-text mb-2">{voiceMode === 'tts' ? 'Text' : 'Describe music'}</p>
            <textarea value={voiceText} onChange={e => setVoiceText(e.target.value)}
              placeholder={voiceMode === 'tts' ? 'Type what to hear...' : 'A chill lo-fi beat...'} rows={3}
              className="w-full py-3 px-4 rounded-xl vs-card border vs-border text-sm vs-text outline-none resize-none"
              style={{ backgroundColor: 'var(--vs-card)' }} />
          </div>
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <textarea value={videoPrompt} onChange={e => setVideoPrompt(e.target.value)}
              placeholder="A drone shot over cyberpunk city..." rows={3}
              className="w-full py-3 px-4 rounded-xl vs-card border vs-border text-sm vs-text outline-none resize-none"
              style={{ backgroundColor: 'var(--vs-card)' }} />
          </div>
      ...
      {confirmClearRecent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-6" onClick={() => setConfirmClearRecent(false)}>
          <div className="vs-card rounded-2xl p-6 max-w-sm w-full text-center border vs-border" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm vs-text-sub mb-5">Non-favorited images will be removed. Your ❤️ favorites are safe.</p>
            <div className="flex gap-2">
          ...
          <div className="vs-card rounded-2xl p-6 max-w-sm w-full border vs-border" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-4">
              ...
              <h3 className="text-lg font-bold vs-text mb-1">
                {keyReason === 'quota' ? 'Pollen depleted' : 'API key required'}
              </h3>
              <p className="text-xs vs-text-sub leading-relaxed">
                {keyReason === 'quota'
                  ? 'Your pollen is depleted. Add your own key to keep generating.'
                  : 'This feature requires your own Pollinations API key.'}
              </p>
            </div>
            <div className="mb-4">
            ...
            {userKey && (
              <div className="mt-4 pt-4 border-t vs-border text-center">
                <p className="text-[10px] vs-text-sub mb-1">Current key: active</p>
            ...
            <button onClick={() => { setShowKeyPopup(false); setPendingAction(null) }}
              className="w-full text-center text-[10px] vs-text-sub hover:underline mt-4">
              Maybe later
            </button>
          </div>
        </div>
